...
find ${SYSTEM_DIR}/components -maxdepth 2 -type f -name Dockerfile -o -name Dockerfile.app |
...
do
  ...
  cat <<-END >> steps.yaml.$$
      - label: ':docker: build - ${module}'
        command: "./${RELATIVE_OVERLORD_PATH}/shared/ci-scripts/build.sh ${module_type}/${module}/${dockerfile_name} build-${BUILDKITE_BUILD_NUMBER}"

...
do
  ...
  lint=$(jq '.scripts.lint' ${module_type}/${module}/package.json)

  if [ "$lint" == "null" ]; then
    echo "no npm run lint. skip"
    continue
  fi
  ...
  cat <<-END >> steps.yaml.$$
      - label: 'npm lint - ${module}'
        command: "./${RELATIVE_OVERLORD_PATH}/shared/ci-scripts/lint.sh ${module_type}/${module}/docker-compose.yaml"

...
do
  ...
  cat <<-END >> steps.yaml.$$
      - label: ':javascript: package - ${module}'
        command: "./${RELATIVE_OVERLORD_PATH}/shared/ci-scripts/package.sh ${module_type}/${module}/docker-compose.yaml"
        artifact_paths:
          - "${module_type}/${module}/build/*.zip"
          - "${module_type}/${module}/build/*.jar"
